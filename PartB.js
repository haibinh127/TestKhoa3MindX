        ...
        $answers[i].onclick = () => {
            ...
            if (!isUserClick) {
                ...
                if ($answers[i].textContent == rightAns) {
                    $tellUser.textContent = `Your answer is correct !`;
                    setTimeout(() => {
                        score++;
                        index++;
                        quizProgram();
                    ...
                    $tellUser.textContent = `Sorry, right answer is ${rightAns}`;
                    setTimeout(() => {
                        index++;
                        quizProgram();
                        $tellUser.textContent = ``;
...
function quizProgram() {
    ...
    if (index < dataArr.length) {
        ...
        $questionTxt.textContent = `Question ${index + 1}:`;
        ...
        $questionTxt.textContent = `Your score is: ${score}`;
        $questionContentTxt.textContent = `Restart Quiz`;
